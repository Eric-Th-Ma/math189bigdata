...
def linreg(X, y, reg=0.0):
    ...
    identity[0,0] = 0
    regId = reg*identity
    W_opt = np.linalg.solve(X.T @ X + regId, X.T @ y)
...
def find_RMSE(W, X, y):
    ...
    RMSE = np.sqrt((np.linalg.norm((y - predict(W, X)),2)**2)/X.shape[0])
    return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    "*** YOUR CODE HERE ***"
    reg_opt = reg_list[0]
    RMSE_min = RMSE_list[0]
    for regIndex in range(len(reg_list)):
        if RMSE_list[regIndex] < RMSE_min:
            RMSE_min = RMSE_list[regIndex]
            reg_opt = reg_list[regIndex]
...
def linreg_no_bias(X, y, reg=0.0):
    '''
        IMPORTANT:
            I COPIED THIS CODE
            I Spent a very long time trying to get this to work but was unable
            to do so (my code was insisting the dimension of my matrix 
            multiplication was wrong), I also tried using the solution to fix
            my code, but because the solution used a different (presumably
            equivalent) equation to find W_opt and b_opt, so in order to 
            continue I copied the solution code, my dode that does not work 
            is pasted here:
    ...
    "*** YOUR CODE HERE ***"
    onesColumn = np.ones(X.shape[0])
    identity = np.eye(X.shape[0])
    regId = reg*identity
    print(X.shape)
    print(onesColumn.shape)
    W_opt = np.linalg.solve((X.shape[0] * (X.T @ X) - 
                             X.T @ onesColumn @ (onesColumn.reshape((-1, 1)) 
                                                   @ X) + 
                             X.shape[0] * regId),
                            (X.shape[0] * (X.T @ y) - 
                             y.T @ onesColumn @ X.T @ onesColumn))
    b_opt = (y.T @ onesColumn - W_opt.T @ X.T @ onesColumn)/y.size
    W_opt = np.linalg.solve(X.T @ X + regId, X.T @ y + b_opt * onesColumn)
...
if __name__ == '__main__':
    ...
    "*** YOUR CODE HERE ***"
    onesColumnTr = np.ones_like(y_train)
    onesColumnTe = np.ones_like(y_test)
    onesColumnV = np.ones_like(y_val)
    X_train = np.hstack((onesColumnTr, X_train))
    X_val = np.hstack((onesColumnV, X_val))
    X_test = np.hstack((onesColumnTe, X_test))
    "*** END YOUR CODE HERE ***"
