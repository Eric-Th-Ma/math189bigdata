...
def grad_descent(X, y, reg=0.0, lr=1e-3, eps=1e-6, max_iter=500, print_freq=20):
...
def newton_step(X, y, W, reg=0.0):
	...
	mu = sigmoid(X @ W)
	diagMu = np.diag(np.squeeze(np.asarray(np.multiply(mu, 1. - mu))))
	H = X.T @ diagMu @ X - reg * np.eye(X.shape[1])
...
def get_description(X, y, W):
	...
	recallNumer = 0.0
	recallDenom = 0.0
	for yIndex in range(len(predictionYs)):
		prediction = predictionYs[yIndex]
		actual = y[yIndex]
		...
		if actual == 1:
			if prediction == 1:
				recallNumer += 1
			recallDenom += 1
	accuracy = accuracyNumer/accuracyDenom
	precision = precisionNumer/precisionDenom
	recall = recallNumer/recallDenom
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0.0, 0.1, 0.5, 1.0, 2.0, 3.0, 5.0, 8.0, 13.0, 21.0, 34.0, 55.0, 89.0]
	for lamb in reg_list:
		W, _ = grad_descent(X_train, y_train, lamb)
